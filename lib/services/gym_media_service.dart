...
class GymMediaService {
  ...
  static const String bucket = 'gyms-images';
  ...
  Future<String> uploadImage(XFile file, String folder) async {
    try {
      ...
      debugPrint(
        '❌ StorageException uploading gym image (bucket=$bucket, folder=$folder): ${e.message}',
      );
      throw Exception('خطأ رفع الصور (Storage): ${e.message}');
    } on PostgrestException catch (e) {
      debugPrint(
        '❌ PostgrestException uploading gym image (bucket=$bucket, folder=$folder): ${e.message}',
      );
      throw Exception('خطأ رفع الصور (DB): ${e.message}');
    } catch (e) {
      if (kDebugMode) {
        debugPrint('❌ Error uploading gym image (bucket=$bucket): $e');
      }
